...
class APIInvoker {
    ...
    invokePUT(url, body, okCallback, failCallback) {
        let params = {
            method: 'put',
            headers: this.getAPIHeader(), body: JSON.stringify(body)
        };
        this.invoke(url, okCallback, failCallback, params);
    }
    ...
    invoke(url, okCallback, failCallback, params) {
        ...
        fetch(`${configuration.api.host}${url}`, params).then((response) => {
            if (debug) {
                console.log("Invoke Response => "); console.log(response);
            }

...
class Reply extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            focus: false, message: '', image: null
        }
    }
        ...
        return (
            <section className="reply">
                ...
                <div className="reply-body">
                    <textarea ref={self => this.reply = self} name="message" type="text"
                        maxLength={config.tweets.maxTweetSize} placeholder="¿Qué está pensando?"
                        className={this.state.focus ? 'reply-selected' : ''} value={this.state.message}
                ...
                <div className={this.state.focus ? 'reply-controls' : 'hidden'}>
                    ...
                    <input href="#" className={this.state.message.length === 0 ? 'btn pull-left disabled' : 'btn pull-left'}
                        accept=".gif,.jpg,.jpeg,.png" type="file" onChange={this.imageSelect.bind(this)} id={"reply-camara-" + randomID}>
                    </input>
                    <span ref="charCounter" className="char-counter"> {config.tweets.maxTweetSize - this.state.message.length}</span>
                    <button className={this.state.message.length === 0 ? 'btn btn-primary disabled' : 'btn btn-primary '}

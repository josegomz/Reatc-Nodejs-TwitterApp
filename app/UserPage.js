...
const UserPage = (props) => {
  ...
  const handleImageChange = (e) => {
    ...
    let id=e.target.id//aqui ocurrio un error de no poder leer el ID dentro de onloadend
    let reader = new FileReader()
    reader.onloadend = (e) => {
      dispath(handleInput(id, reader.result))
    }
    reader.readAsDataURL(file)
  }

  //manejador de archivos de texto
          ...
          <div className="user-banner" style={{ backgroundImage: 'url(' + (profile.banner) + ')' }}>
            <If condition={edit}>
              <div>
                ...
                <input href="#" className="btn" accept=".gif,.jpg,.jpeg,.png" type="file" id="banner" onChange={handleImageChange} />
              </div>
            </If>
          </div>
